...
Characteristic for data science is that of requires combining many different techniques, skills, and perspectives. Obviously, no-one can really claim to have mastered all the different aspects displayed in this figure (and this figure is by no means exhausitve, which means there are many more items that could easily be added to this sketch!). In many cases, it is mostly about learning important key competences but also learning how to navigate in areas where one has not yet fully mastered the skills and instead is able to either acquire new knowledge when nedded or (often better) be able to find the right people to help, for instance in form of collaboration, team work, or discussions.
```
